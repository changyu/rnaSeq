...
if(strlen($current_user->user_login)!=5){
...
}elseif(!file_exists($_POST['gtf'])){
	echo "<h1>".$_POST['gtf']." not exist!</h1>";
}elseif((substr_count($lbls,',')+1)!=sizeof($_POST['sbam'])){
	echo "<h1>Number of labels must match number of groups.</h1><h1> Here:  ".(substr_count($lbls,',')+1)." labels vs ".sizeof($_POST['sbam'])." groups</h1>";
}else{
   $okey=1;
  ...
  // echo "size ==".sizeof($sbams)."<br>";
   ...
   $jid=substr($proj, 0, strpos($proj, "."))?substr($proj, 0, strpos($proj, ".")):$proj;
   ...
   if(strlen($mis)<1){
	...
	for($i=0;$i<sizeof($sbams);$i++){
		for($j=0;$j<sizeof($seqs[$i]);$j++){
			...
			$cfqs.="cuffquant -p 4 --library-type $libt  -o $fd  $gtf ".$seqs[$i][$j]." & \n";
	...
	$output="Results";
        $cfqs=$cfqs."wait\n";
	#echo $cfqs; exit();
        $cnm="cuffnorm -p 4 --library-type $libt  -o $proj_dirname/cuffnorm -L $lbls  $gtf $fdir\n";	
